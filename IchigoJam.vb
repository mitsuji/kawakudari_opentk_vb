...
Class Std15
  ...
  Sub New (screenW As Integer, screenH As Integer, buffW As Integer, buffH As Integer)
    Me.screenW = screenW
    Me.screenH = screenH
    Me.buffW = buffW
    Me.buffH = buffH
  ...
  Private Sub DrawRect(x As Single, y As Single, w As Single, h As Single)
    Dim x0 As Single = (2.0f / screenW) * x - 1.0f
    Dim y0 As Single = 1.0f - (2.0f / screenH) * y
    Dim x1 As Single = x0 + (2.0f / screenW) * w
    Dim y1 As Single = y0 - (2.0f / screenH) * h
  ...
  Public Sub DrawChar (cx As Integer, cy As Integer, c As Char)
    ...
    For y As Integer = 0 TO CHAR_H -1
      ...
      For x As Integer = 0 TO CHAR_W -1
        If ((line >> (CHAR_W-x-1)) And &h1) = &h1 THen
          GL.Color4(1.0f,1.0f,1.0f,1.0f) '' White
          DrawRect((cx*CHAR_W+x)*dotW, (cy*CHAR_H+y)*dotH, dotW, dotH)
        End If
      Next
    Next
  End Sub
